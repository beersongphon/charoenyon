...
<body>
  ...
  <div class="row p-4">
    ...
    ?>
      <div class="col-9">
        <center>
          <h2>เพิ่มรายการประกันภัย</h2>
          <form method="POST" action="" style="width:50%; text-align: left;">
            <div class="form-group">
              <label>ชื่อ</label>
              <input class="form-control" name="name" type="text" placeholder="กรอกชื่อประกันภัย" required value="<?php echo $row["name"]; ?>">
            </div>
    ...
    if ($_POST) {
      $name = $_POST['name'];
      $sql = "UPDATE insurance SET name = '" . $name . "' WHERE insurance_id = " . $id;
      $result = $conn->query($sql);
      if ($result == TRUE) {
        echo "<script>
