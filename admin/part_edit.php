          ...
          <center>
            ...
            <form method="POST" action="" enctype="multipart/form-data" style="width:50%; text-align: left;">
              <div class="form-group">
                <label>ชื่อ</label>
                <input class="form-control" id="parts_name" name="parts_name" type="text" placeholder="กรอกชื่ออะไหล่" required value="<?php echo $row["parts_name"]; ?>">
              </div>
              <div class="form-group">
                <label>ราคา</label>
                <input class="form-control" id="parts_price" name="parts_price" type="number" placeholder="กรอกราคาต่อชิ้น" required value="<?php echo $row["parts_price"]; ?>">
              </div>
              <div class="form-group">
                ...
                <select class="form-control" id="company_parts_id" name="company_parts_id" required>
                  ...
                  ?>
                    <option value="<?php echo $company["company_parts_id"]; ?>" <?php if ($row["company_parts_id"] == $company["company_parts_id"]) echo 'selected'; ?>>
              ...
              <div class="form-group">
                ...
                <select id="parts_year" name="parts_year" class="form-control" required>
                  <option value="1990-1995" <?php if ($row['parts_year'] == "1990-1995") echo 'selected'; ?>>1990-1995</option>
                  <option value="1996-2000" <?php if ($row['parts_year'] == "1996-2000") echo 'selected'; ?>>1996-2000</option>
                  <option value="2001-2006" <?php if ($row['parts_year'] == "2001-2006") echo 'selected'; ?>>2001-2006</option>
                  <option value="2007-2010" <?php if ($row['parts_year'] == "2007-2010") echo 'selected'; ?>>2007-2010</option>
                  <option value="2011-2016" <?php if ($row['parts_year'] == "2011-2016") echo 'selected'; ?>>2011-2016</option>
                  <option value="2017-2021" <?php if ($row['parts_year'] == "2017-2021") echo 'selected'; ?>>2017-2021</option>
                  <option value="2022-2025" <?php if ($row['parts_year'] == "2022-2025") echo 'selected'; ?>>2022-2025</option>
                </select>
              </div>
              <div class="row">
                <div class="col-6">
              ...
              <button class="btn btn-primary btn-block" type="button" onclick="editAutoPart()">
                บันทึก
              </button>
            </form>
          </center>
...
<script>
  ...
  $(document).ready(function() {
    let parts_id = $('#parts_id').val();
    $.ajax({
      url: 'query/get_all_image_auto_part.php',
      type: 'post',
      data: {
        'parts_id': parts_id
      },
  ...
  function editAutoPart() {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response) {
          $.ajax({
            url: 'query/delete_image_auto_part.php',
            type: 'post',
            data: {
              'parts_id': parts_id
            },
  ...
  async function addImage(image, parts_id) {
    await $.ajax({
